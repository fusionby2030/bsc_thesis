...
% Preamble
\documentclass[a4paper, twoside, BCOR1.4cm, headsepline, DIV12, appendixprefix, final, 12pt]{article}
...
The topic of this thesis is to explore the use cases of state of the art ML tools in predicting specific plasma parameters, specifically the plasma in the edge region of tokamaks.
...
\begin{figure}
	...
	\begin{subfigure}{0.3\linewidth}
		\centering
		\includegraphics[scale=0.4]{./src/plasma_shaping}
		\caption{}
		\label{subfig:plasmashaping}
	\end{subfigure}\hfill
	\begin{subfigure}{0.22\linewidth}
		\centering
		\includegraphics[scale=0.4]{./src/mtanh_94962temperature_error}
		\caption{}
		\label{subfig:ILW_mtanh}
	\end{subfigure} \hfill
	\begin{subfigure}{0.45\linewidth}
		\centering
		\includegraphics[scale=0.15]{./src/diverter_config}
		\caption{}
...
The fit uncertainties are expected to be significantly smaller than the natuMTANHral scatter of the data due to the fluctuation of the plasma.
...
After filtering out the RMPs, Kicks, Pellets, non-validated HRTS, and shots that do not use deuterium, the dataset is reduced to 1888 entries, and the control parameter range and pedestal parameter range are given in the tables below.
\begin{center}
\begin{tabular}{| c | c | }
	\hline
	Eng. Param & Domain \\
...
The choice of parameters from the log-linear regression by Lorenzo et. al., was backed by physical intuition about what drives the pedestal density height, as well as an avoidance of using cross-correlated variables.
In order to improve prediction quality, it may prove useful to include additional inputs from the list of main engineering quantities.
...
However, this is a case of ice-cream correlation\footnote{ice cream correlation refers to the correlation of increasing ice cream sales and increasing drowning rate in Finland during the summer. Although the variables are indeed correlated, ice cream is not in fact the causation of drowning, nor vice-versa.}, and the real culprit of the causation is \textcolor{green}{expand Shafranov shift[SOURCE]}.
...
\begin{figure}
        ...
        \begin{subfigure}{0.33\linewidth}
                \centering
                \includegraphics[scale=0.4]{./src/R_vs_NEPED_DC}
                \caption{}
                \label{subfig:rvsneped}
        \end{subfigure} \hfill
        \begin{subfigure}{0.66\linewidth}
                \centering
                \includegraphics[scale=0.3]{./src/correlation_shit}
                \caption{}
                \label{subfig:corr_shit}
        \end{subfigure}
	\caption{(a) the 'ice cream corerlation' between the major radius $R$, divertor configuration, and $n_e^{ped}$,  (b) RHS: the correlation matrix of the engineering parameters,  LHS: the Spearman rank-coefficient (SRC)[SOURCE] tree of main engineering parameters. Both correlation visuals are particularly useful in determining viable input parameters for the ML models that do work well with highly correlated features, as well as determining viable input par  }
	\label{fig:emperical}
\end{figure}
...
\begin{itemize}
	...
	\item Sensitivity analysis is used to determine the relevant engineering parameters for GPs, from which the dimensionality of the input space can be reduced if parameters are deemed irrelevant. Three different forms of sensitivity analysis used: KLD, ARD, VAR[SOURCE], which all make use of thte length-scale parameters of the kernel in a GP model.
\end{itemize}
...
The three types of sensitivity analysis are briefly outlined below:
	\begin{itemize}
...
Each model analysed is fit using the following list of control parameters as inputs: 
\begin{itemize}
...
\begin{figure}
	\begin{subfigure}{0.33\linewidth}
		\centering
		\includegraphics[scale=0.35]{ ./src/GP_ARD_vs_KL_VAR_feature_relevance}
		\caption{The result of the the three forms of sensitivity analysis.}
		\label{subfig:ARD_KL_VAR}
	\end{subfigure}
	\begin{subfigure}{0.33\linewidth}
		\centering
		\includegraphics[scale=0.2]{ ./src/GP_kernels_num_features}
		\caption{}
...
I would expect this to change if a wider range of fuelling elements were included in the dataset, and for future work do not expect $M_{eff}$ to rank as low as it does. Each sensitivty analysis also ranks $B_T$ very low, which is most likely due to the inherent correlation between $B_T, q_{95}, \text{and} I_P$. 

Using the results from the sensitivity analysis, the performance vs dimensionality was measured for four homoscedastic GPs with varying kernels and plotted in \ref{subfig:GP_vs_dimensionality}.
Starting with a dimensionality of 1 means that only $\delta$ was supplied for fitting, and a dimensionality of 2 was with $\delta,a$, and so on following the ranking of the KLD (averaging all three analysis follows the KLD relevance rankings). 
The fitting procedure for each dimensionality made use of the repeated fold CV process, with 7 folds and 4 repeats, then averaging the performance across the 28 different test sets.
We can see from Figure \ref{subfig:GP_vs_dimensionality} that the performance of each kernel generally reaches a relative minimum when it reaches dimension 8, i.e., not much more information is gained after including $B_T$. However, for the top two performing kernels, Rational Quadratic and MLP, the ultimate minimum in RMSE occurs when all inputs are included, albeit the decrease in RMSE is very small, so much so that it could be argued to be statistical. If computational cost is to be limited or speed is of the essence, then removing $P_{ICRH}, H, M_{eff}$ will certainly help, without too much degredation to prediction quality. 

To determine the effect of uncertainty propagation, a fixed (is not optimized) noise kernel was added on to a MLP kernel (the top performing kernel determined from the sensitivity analysis), transforming the noise term in the GP model from homoscedastic to heteroscedastic.
...
Through analysing various machine learning models, it is clear that there can be improvements to predicting the pedestal density height while only using main engineering parameters.
