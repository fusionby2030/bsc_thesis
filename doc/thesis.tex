...
The ANN analysis in this paper is constrained to the use of shallow networks ($\leq 6$ hidden layers) for reasons seen in Figure \ref{fig:ANN_dim}. 
Throughout the hyperparameter optimization process, each model was trained and tested using the same repeated cross-fold validation methods, with 5 folds and 5 repeats. Typically, the first hyperparameters to be optimized are the learning rate (LR) and mini-batch size (MBS), and the optimal values are those that minimize the MSE on the training set. Through random search, the optimal MBS and LR were found to be 396 and 0.004 respectively. Considering the dataset is around 2000 entries, the mini-batch size is relatively large, and the learning rate could be considered very small for such a large mini-batch size. However, since each training epoch, the data is randomly shuffled, it is possible that the training samples in each batch 'compete' with eachothers gradient. For example, the training samples of $n_e^{ped} \geq 9.5$ may want to pull the model weights in an opposite direction than that $n_e^{ped} \leq 9.5$. The gradient updates applied for this mini-batch size seemed to balance the effect of the training samples, thus resulting in the best training/testing performance. 

Then, via grid search (across all available activation layers offered by Pytorch), the optimal activation function was deterimend to be ELU \textcolor{green}{(alpha?)}. 

Using these hyperparameters, models of varying sizes of hidden layers were fit, and as seen in Figure \ref{fig:ANN_dim}, the initial ansatz of hidden layers was determined to be either 3 or 4, with between 1000-2000 nodes (split between the each of the hidden layers). This criteria was used as a space for further architecture search via random search, and the optimal network was found to have 4 layers of, hidden sizes of [69, 37, 31, 18] (in order of 1st hidden layer onward). 
...
\begin{figure}
	...
	\begin{subfigure}{0.495\linewidth}
		...
		\includegraphics[scale=0.4]{example-image-a}
		\caption{}
		\label{subfig:ANN_dim_sim}
	\end{subfigure}
	\begin{subfigure}{0.495\linewidth}
		\centering
		\includegraphics[scale=0.4]{example-image-b}
		\caption{}
		\label{subfig:ANN_dim_ens}
	\end{subfigure}\hfill
	\caption{(a) Performance vs Size of ANN (b) Performance vs Size of ANN Ensembles}
	\label{fig:ANN_dim}
\end{figure}
\begin{figure}
